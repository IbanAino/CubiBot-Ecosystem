#include "LidarProtocol.h"

namespace lidar {

namespace {
static const uint8_t CrcTable[256] = {
    
};
} // namespace

ProtocolParser::ProtocolParser()
    : rxIndex_(0)
{
}

void ProtocolParser::reset()
{
    rxIndex_ = 0;
}

uint8_t ProtocolParser::calculateCRC8(const uint8_t* data, size_t length)
{
    uint8_t crc = 0;

    for (size_t i = 0; i < length; ++i) {
        crc = CrcTable[(crc ^ data[i]) & 0xFF];
    }

    return crc;
}

bool ProtocolParser::pushByte(uint8_t byte, uint8_t* frame)
{
    // Recherche du header
    if (rxIndex_ == 0) {
		
        if (byte != HEADER) {
            return false;
        }
    }

    // Vérification du deuxième octet
    else if (rxIndex_ == 1) {

        if (byte != VERLEN) {

            rxIndex_ = 0;

            // Ce byte peut être le début d'une nouvelle trame
            if (byte == HEADER) {
                rxIndex_ = 1;
                frame[0] = HEADER;
            }

            return false;
        }
    }

    // Stockage du byte reçu directement dans la frame
    frame[rxIndex_] = byte;
    rxIndex_++;

    // Trame complète
    if (rxIndex_ == FRAME_LENGTH) {

        rxIndex_ = 0;

        return true;
    }

    return false;
}

} // namespace lidar